...
new<- cbind(new, dataset[c('Clicked.on.Ad')])
head(new)
```
...
```{r}
# Installing caret for training our model.
library(caret)
...
set.seed(42)
intrain <- createDataPartition(y = new$Clicked.on.Ad, p= 0.8, list = FALSE)
...
```{r}
# Checking the dimensions in our train and test sets.
dim(training); 
dim(testing);

```
```{r}
# Factorizing our target variable to convert the values(0 and 1) to categorical variables.

training[["Clicked.on.Ad"]] = factor(training[["Clicked.on.Ad"]])
```

```{r}
# Cross validating before training our model.
...
```{r}
# Predicting results in our testing data and checking the accuracy and classification matrix to evaluate the model's performance.
test_pred <- predict(svm_Linear, newdata = testing)
confusionMatrix(table(test_pred, testing$Clicked.on.Ad))
```
We have a high accuracy score of 96%. From our classification matrix we have 5 false positives and 3 false negatives.

Naive Bayes

```{r}
# Converting the output variable into a categorical variable
new$Clicked.on.Ad <- factor(new$Clicked.on.Ad, levels = c(0,1), labels = c("False", "True"))
```

```{r}
# Splitting data into training and test data sets
set.seed(42)
indxTrain <- createDataPartition(y = new$Clicked.on.Ad,p = 0.8,list = FALSE)
training <- new[indxTrain,]
testing <- new[-indxTrain,]

```

```{r}
# x holds the predictor variables and y holds the response variable
x = training[,-6]
y = training$Clicked.on.Ad
```

```{r}
# Loading the library that holds the Naive Bayes function.
library(e1071)
```


```{r}
# Cross validating and building our model.
...
```{r}
# Checking the model accuracy and the confusion matrix.
confusionMatrix(Predict, testing$Clicked.on.Ad )
```

Our Naive Bayes model has a high accuracy score of 95.5%. From our classification matrix we have 5 False Positives and 4 False Negatives.
